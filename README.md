...
**Features**
- **Side-by-side FP32 vs INT8** inference with server-measured latency and an "agree" check on every image (sample, upload, or draw-your-own).
- **Grad-CAM overlay** showing which pixels drove the prediction.
- **Throughput / latency benchmark** endpoint (per-run latencies, ms and img/s for each model).
- **Noise-robustness slider** that perturbs the input and re-classifies.
- **Confusion matrix + per-class accuracy** for the INT8 model over the full test set.
- **Model card** section (architecture, quantization config, dataset, limitations).

**Run locally**
```bash
pip install -r server/requirements.txt
uvicorn server.main:app --port 8000        # from the repo root
# open http://localhost:8000
```

**Run with Docker**
```bash
docker build -t edge-vision-quant . && docker run -p 8000:8000 edge-vision-quant
```

The web assets and the confusion matrix are generated by [`export_web.py`](export_web.py). The API lives in [`server/main.py`](server/main.py); a static ONNX export of the FP32 model is also produced for portability.
